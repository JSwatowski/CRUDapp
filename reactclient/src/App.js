...
export default function App() {
  ...
  function getPosts() {
    const url = 'https://localhost:44334/api/Post';
  ...
  function deletePost(postId) {
    const url = `https://localhost:44334/api/Post/delete/${postId}`;
      ...
      <div className="row min-vh-100">
        <div className="col d-flex flex-column justify-content-center align-items-center">
          {(showingCreateNewPostForm === false && postCurrentlyBeingUpdated === null) && (
            <div>
              <h1>Blog with random comments</h1>
  ...
  function renderPostTable() {
    return (
      <div className="table-responsive mt-5 ">
          ...
          <tbody>
            {posts.map((post) => (
              <tr key={post.postId}>
                ...
                <td>{post.text}</td>
                <td>
                  <Button onClick={() => setPostCurrentlyBeingUpdated(post)} className="btn btn-dark btn-lg mx-3 my-3"icon="k-icon k-i-edit">Update</Button>
                  <Button onClick={() => { if (window.confirm(`Are you sure to delete post from "${post.userName}"?`)) deletePost(post.postId) }} className="btn btn-secondary btn-lg" icon="k-icon k-i-delete">Delete</Button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <button onClick={() => setPosts([])} className="btn btn-dark btn-lg w-100 mt-3">Close posts</button>
